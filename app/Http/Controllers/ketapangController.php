...
class KetapangController extends Controller
{

    public function ketapang1(Request $request)
    {
        ...
        $tbl65a=DB::table('ketapang')->where('status', '=', 'Accepted')->get();
        $ketapang=0;
       ...
       //kesehatan jumlah dokter
        ...
        if($request->has('cari')){
            ...
            return view("ketapang.ketapang",  compact('ketapang','tbl65','categories65','data65','tbl61','tbl65a',
 ...
 $categories39 = [];
       ...
       $categories40 = [];
        ...
        foreach ($tbl53 as $tabel52){
            ...
            return view("pages.bantuanpemerintahkecamatan",  compact('ketapang','tbl65','categories65','data65','tbl65a',
...
public function formulir1(){
            return view("pages.formulir-bantuan-pemerintah-kecamatan");
}

public function tambah1(Request $request)
    {
        ketapang::create(['jenis_bantuan'=> $request->jenis_bantuan, 'keterangan'=>$request->keterangan,
        'kecamatan' => $request->kecamatan, 'desa'=>$request->desa, 'jumlah'=>$request->jumlah,
        'tahun'=>$request->tahun]);
        return back()->withStatus(__('Data Berhasil Disimpan'));
    }

    public function hapus65($id)
{
	DB::table('ketapang')->where('id',$id)->delete();
	return redirect('/bantuan_pemerintah_kecamatan');
}

    public function edit65($id)
    {
            $tabel65 = ketapang::FindOrFail($id);
            return view('pages.edit.ketapangedit',compact('tabel65'));
    }

    public function update65(Request $request, $id){
        ...
        ketapang::where('id',$id)->update([
            'jenis_bantuan'=>$input_data['jenis_bantuan'],
        ...
        return redirect('/bantuan_pemerintah_kecamatan')->with('message','Update already Update!');
    }

    public function exportpdf65()
    {
        $ketapang = ketapang::all()->where('status','=','Accepted');
        $pdf = PDF::loadView('pages.export.ketapang_pdf', ['ketapang' => $ketapang]);
        return $pdf->download('ketapang.pdf');
    }
